...
static void	size_management_unsigned(void)
{
	...
	tmp = 0;
	if (g_current_data.size == HH)
{		tmp = (unsigned long long int)((unsigned char)va_arg(g_current_data.list, unsigned int));
}	else if (g_current_data.size == H)
{		tmp = (unsigned long long int)((unsigned short int)va_arg(g_current_data.list, unsigned int));
}	else if (g_current_data.size == L)
